      ...
      <div className="space-y-4">
        {orderItems.map((pro) => (
          <div key={pro._id} className="flex items-center justify-between border-b pb-4">
            <img src={pro.imageUrl} alt={pro.name} className="h-24 w-24 object-cover rounded-md" />
            <div className="ml-4 flex-1">
              <p className="text-lg font-medium text-gray-800">{pro.name}</p>
              <p className="text-gray-600">Quantity: <span className="font-semibold">{pro.quantity}</span></p>
              <p className="text-gray-600">Price: <span className="font-semibold">₹{pro.price.toFixed(2)}</span></p>
            </div>
